...
if (user.joincount < 1) throw `${ag}*¡YA NO TIENES TOKENS! 🪙*\n\n*COMPRA TOKENS PARA PODER INVITAR A BOT A TÚ GRUPO CON EL COMANDO ${usedPrefix}buy3 o ${usedPrefix}buyall3* *cantidad*`
 //if (user.joincount === 0 ) throw `${ag}*¡YA NO TIENES TOKENS! 🪙*\n\n*COMPRA TOKENS PARA PODER INVITAR A BOT A TÚ GRUPO CON EL COMANDO ${usedPrefix}buy3* *cantidad*`  
 ...
 if (args[1] < 3) throw `${fg}*MÁXIMO 3 TOKEN(S) PARA PODER INVITAR A BOT*`
 if (args[1] > 3) throw `${fg}*MÍNIMO 3 TOKEN(S) PARA PODER INVITAR A BOT*` //Solo ingresará si tiene 3 Token(s)
 
let res = await conn.groupAcceptInvite(code) 
conn.reply(m.chat, `${eg}😻 *SE HA UNIDO ${wm} AL GRUPO!!!*`, m).then(async() => { 
...
if (!linkRegex.test(args[0])) throw `${fg}𝙀𝙉𝙇𝘼𝘾𝙀 𝙉𝙊 𝙑𝘼𝙇𝙄𝘿𝙊.` //Aquí 
  ...
  let res = await conn.groupAcceptInvite(code) 
  conn.reply(m.chat, `${eg}😻 *SE HA UNIDO ${wm} AL GRUPO!!!*`, m).then(async() => { //Si el Owner se une al Grupo no habrá temporizador
let img = 'https://i.imgur.com/8fK4h6F.jpg'
